...
4. При любых обстоятельствах ЦЕКЭОБ получает только аутентичные данные с датчиков
5. При любых обстоятельствах обеспечивается конфиденциальность данных с датчиков
6. При любых обстоятельствах ЦЕКЭОБ получает только аутентичные координаты судна
...
|:---|:--|:--|
|Модуль шифрования|Расшифрование/шифрование принимаемых/отправляемых пакетов данных.|Работает при помощи SSL протокола. Предполагается, что в ОрВД и Системе ЦЕКЭОБ тоже реализован данный функционал|
|Обработчик сообщений|Распределение задач.|Отправляет параметры маршрута в расчет перемещения, контроль маршрута и контроль выполнения задания. Также занимается передачей сообщений для их последующей отправки с судна|
|Расчет перемещения|Расчет курса |Рассчитывает необходимые курс и скорость вращения винта для достижения очередной точки маршрута|
|Контроль перемещения|Проверка работы модуля расчета перемещения|Вычисляет куда судно придет изпользуя инструкции полученные с модуля расчета перемещения и отправляет примерное положение судна в контроль машрута|
|Контроль маршрута|Проверка правильности выполнения маршрута.|Контролирует выполнение задачи, сверяет текущие координаты с маршрутом, передает команду в модуль аварийной остановки в случае расхождения.|
|Система навигации GPS|Физический модуль GPS, сообщающий координаты судна.||
|Внутренняя навигация|Физический модуль ИНС, сообщающий координаты судна.||
|Комплексирование|Комплексирование навигационных данных| Комплексирование навигационных данных, а также отправка текующих навигационных данные в модуль контроля маршрута|
|Контроль выполнения задания|Сверяет навигационные данные с маршрутом.|В случае достижения точки сбора данных дает соответствующую команду модулю датчиков.|
|Отправка телеметрии|Отправка сообщения с данными о телеметрии в модуль обработки сообщений.||

![Переработанная архитектура](images/black_arch.png)
...
![Переработанная архитектура](images/LEGENDA.png)
...
|2. Модуль шифрования |$\textcolor{orange}{\textsf{Повышающий доверие}}$|SS|2|Модуль реализующих криптографические функции|
|3. Обработка сообщений|$\textcolor{green}{\textsf{Доверенный}}$|MM|3|Отвечает за прием-отправку сообщений, обрабатывает, преобразовывает и отправляет сообщения по модулям|
|4. Расчет перемещения|$\textcolor{red}{\textsf{Недоверенный}}$|CL|1|Модуль вычисляющий правила и иструкции по движению на маршруте, занимается матиматическим вычислением пути|
|5. Контроль перемещения|$\textcolor{orange}{\textsf{Повышающий доверие}}$|SS|1|Ограничетель вычисляющий предполагаемое местополажение корабля и сравнивающий полученные вычисления с маршрутом|
|6. Приводы|$\textcolor{red}{\textsf{Недоверенный}}$|MM|2|Сервоприводы и драйверы|
|7. Контроль маршрута|$\textcolor{green}{\textsf{Доверенный}}$|SM|4|Сравнивает текущее состояние лодки с данными приходящими от других модулей|
|8. Контроль заряда батареи|$\textcolor{green}{\textsf{Доверенный}}$|SS|0|Модуль считывающий батарею|
|9. Система навигации GPS|$\textcolor{red}{\textsf{Недоверенный}}$|CL|0|Аппаратный модуль спутниковой системы связи|
|10. Внутренняя навигация (Инерциальная система)|$\textcolor{orange}{\textsf{Повышающий доверие}}$|CL|0|Аппаратный модуль инерциальной навигационной системы|
...
|12. Контроль выполнения задания|$\textcolor{green}{\textsf{Доверенный}}$|SS|2|Модуль сравнивающих текущее положение лодки с координами задания и отдающий команду датчикам сбора|
|13. Датчики|$\textcolor{green}{\textsf{Доверенный}}$|SM|1|Аппаратный модуль контроля параметров окружающей среды|
|14. Отправка телеметрии|$\textcolor{green}{\textsf{Доверенный}}$|SS|2|Модуль, занимающийся подготовкой координат и показаний датчикой в обработку сообщений|
|15. Аварийная остановка|$\textcolor{green}{\textsf{Доверенный}}$|SM|1|Модуль останавливающий движение судна по команде модуля контроля маршрута перестает подавать питание на приводы(отрезаем провода)|
...
1) Монитор безопасности и брокер сообщений - доверенные модули
2) В некоторых модулях (например шифрование) , функции будут представлять из себя API или заглушку для простоты реализации
3) Между собой сервисы общаются через шину сообщений (message bus), а всё снаружи принимают в виде REST запросов
4) В данном учебном примере реализация модуля GPS и ИНС не отличается, из-за отсутствия аппаратной реализации
