...
|3. Обработчик сообщений|Распределение задач.|Отправляет параметры маршрута в рассчет перемещения, контроль маршрута и контроль выполнения задания. Также занимается передачей сообщений для их последующей отправки с судна|
|4. Рассчет перемещения|Рассчет курса. |Рассчитывает необходимые курс и скорость вращения винта для достижения очередной точки маршрута|
|5. Контроль перемещения|Проверка работы модуля рассчета перемещения|Вычисляет куда судно придет изпользуя инструкции полученные с модуля расчета перемещения и отправляет примерное положение судна в контроль машрута|
|6. Приводы|Модуль был описан выше||
|7. Контроль маршрута|Проверка правильности выполнения маршрута.|ПКонтролирует выполнение задачи сверяет текущие координаты с маршрутом, передает команду в модуль аварийной остановки в случае расхождения.|
...
|12. Контроль выполнения задания|$\textcolor{green}{\textsf{Доверенный}}$|При компрометации нарушаются ЦБ 4-5|Сверяет текущие координаты с коордиатами задания и дает команду датчикам для забора анализов|
...
|НС-3|Тест Негативного сценария 11, 12. Злоумышленник взламывает модуль "Расчет перемещения"/"приводы", но модуль "Контроль перещения" фиксирует отклонение от маршрута и судно совершает аварийную остановку, ЦБ не нарушены|
|НС-4|Тест Негативного сценария 13, 14. Злоумышленник cкомпрометировал модуль "Система навигации GPS", но модуль "Комплексирования" сравнил результаты с данными модуля "Внутренняя навигация (Инерциальная система)" и выявил взлом, ЦБ не нарушены|
