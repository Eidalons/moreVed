...
def read_coords(error):
    ...
    print("[GNSS_DEBUG] Readed coords:", coords)
...
def generate_coordinates():
    while True:
        ...
        print("[internal_navigation] Coords:" , coords)
        proceed_to_deliver(uuid4().__str__(),
                           {
                               "deliver_to": "complex",
                               "operation" : "set_internal_coords",
                               "coords": coords
                           })
        sleep(randint(5,10))
...
def start_producer(args , config, requests_queue):
    ...
    threading.Thread(
        target=lambda: producer_job(args, config, requests_queue)).start()